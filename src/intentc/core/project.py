...
class Project(BaseModel):
    ...
    def resolve_implementation(self, name: str | None = None) -> Implementation | None:
        ...
        raise ValueError(
            f"Ambiguous: multiple implementations exist ({list(self.implementations.keys())}). "
            "Specify one explicitly or name one 'default'."
        )
    ...
    def topological_order(self) -> list[str]:
        ...
        if len(order) != len(self.features):
            ...
            raise ValueError(f"Cycle detected in feature DAG involving: {missing}")
...
def load_project(intent_dir: Path) -> Project:
    ...
    for ic_file in sorted(intent_dir.rglob("*.ic")):
        ...
        parts = rel.parts
        if parts[0] in skip_dirs or rel.name == "project.ic":
            continue
        # Feature path: all directory parts up to the feature directory
        # e.g., intent/core/specs/feature.ic -> "core/specs"
    ...
    all_feature_paths = sorted(features.keys())
...
def write_project(project: Project, dest_dir: Path) -> Path:
    ...
    for vf in project.assertions:
        if vf.source_path is not None:
            name = vf.source_path.name
        else:
            name = f"{vf.target.replace('/', '_')}.icv"
    ...
    for fp, node in project.features.items():
        ...
        for intent in node.intents:
            if intent.source_path is not None:
                name = intent.source_path.name
            else:
                name = f"{intent.name.replace('/', '_')}.ic"
...
def blank_project(name: str) -> Project:
    ...
    starter_impl = Implementation(
        name="default",
        body="# Default Implementation\n\nDescribe how this project is built.\n",
    )
    ...
    return Project(
        project_intent=project_intent,
        implementations={"default": starter_impl},
