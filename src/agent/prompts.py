...
def _format_validation_file(vf: object) -> str:
    """Format a validation file reference for inclusion in a prompt."""
    from agent.types import ValidationFileRef

    if not isinstance(vf, ValidationFileRef):
        return str(vf)
    parts = [f"target: {vf.target}"]
    for v in vf.validations:
        parts.append(f"- {v.name} ({v.type}, {v.severity}): {v.args}")
    return "\n".join(parts)
