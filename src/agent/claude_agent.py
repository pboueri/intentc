"""Claude Code agent (standalone copy)."""
...
class ClaudeAgent(Agent):
    ...
    def _run_non_interactive(self, prompt: str, output_dir: str = "") -> None:
        ...
        cmd.extend(["-p", prompt, "--verbose", "--output-format", "stream-json", "--dangerously-skip-permissions"])
        ...
        try:
            ...
            if proc.returncode != 0:
                raise AgentError(f"Claude process exited with code {proc.returncode}")
    ...
    def plan(self, ctx: BuildContext) -> None:
        ...
        try:
            subprocess.run(cmd, timeout=self._profile.timeout)
        except subprocess.TimeoutExpired as exc:
            raise AgentError(f"Planning session timed out after {self._profile.timeout}s") from exc
